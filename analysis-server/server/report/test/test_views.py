...
class TestReportEvent:
    def test_report_events_event_report_calculate_ok(self, client, event1_for_report_api):
        url = reverse(viewname="report_events", args=[event1_for_report_api.id, ])
...
class TestReportStore:
    def test_report_stores_store_report_calculate_ok(self, client, store1_for_report_api):
        url = reverse(viewname="report_stores", args=[store1_for_report_api.id, ])
        response = client.get(url)
