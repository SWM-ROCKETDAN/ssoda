...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(authInterceptor)
                ...
                .addPathPatterns("api/v1/users/**", "/api/v1/events/hashtag/**", "/api/v1/stores/**", "/api/v1/rewards/**",  "/api/v1/report/**")
                // /api/v1/login/** 으로의 유입은 인터셉터를 거치지 않는다. (로그인 시도는 인증에 상관없이 가능하다,)
                .excludePathPatterns("api/v1/login/**", "/api/v1/events/{id}", "/api/join/**");
    }
