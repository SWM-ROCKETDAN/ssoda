...
public class RankService {
    ...
    public EventRankResponseDto wrapEventRank(EventRankDto eventRankDto) {
        Event event = eventRepository.findById(eventRankDto.getEvent_id()).orElseThrow(() -> new IllegalArgumentException("해당 이벤트가 없습니다. id=" + eventRankDto.getEvent_id()));
        Store store = event.getStore();
        return new EventRankResponseDto(event, store, eventRankDto);
    }
